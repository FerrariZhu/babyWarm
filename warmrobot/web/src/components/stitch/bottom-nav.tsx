...
const tabs = [
  { href: "/", label: "首页", action: "home", icon: "home", filledWhenActive: true },
  { href: "/records", label: "穿衣记录", action: "records", icon: "event_note", filledWhenActive: true },
  { href: "/profile", label: "我的", action: "profile", icon: "person", filledWhenActive: true },
] as const;
...
export function BottomNav() {
  ...
  return (
    <nav
      aria-label="主导航"
      data-analytics-module="bottom_navigation"
        ...
        {tabs.map((tab) => {
          ...
          return (
            <li key={tab.href} className="flex min-w-0 flex-1">
              <Link
                href={tab.href}
                data-analytics-action={tab.action}
                prefetch={true}
